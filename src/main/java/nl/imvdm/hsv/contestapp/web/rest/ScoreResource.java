...
/**
 * REST controller for managing {@link nl.imvdm.hsv.contestapp.domain.Score}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ScoreResource {
    ...
    public ResponseEntity<Score> partialUpdateScore(
        ...
        Optional<Score> result = scoreRepository
            ...
            .map(existingScore -> {
                if (score.getPunten() != null) {
                    existingScore.setPunten(score.getPunten());
                }
     ...
     */
    @GetMapping("/scores")
    public List<Score> getAllScores(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Scores");
        return scoreRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Score> getScore(@PathVariable Long id) {
        ...
        Optional<Score> score = scoreRepository.findOneWithEagerRelationships(id);
