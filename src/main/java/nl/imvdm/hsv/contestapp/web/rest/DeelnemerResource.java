...
public class DeelnemerResource {
    ...
    public List<Deelnemer> getAllDeelnemers(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all Deelnemers");
        return deelnemerRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<Deelnemer> getDeelnemer(@PathVariable Long id) {
        ...
        Optional<Deelnemer> deelnemer = deelnemerRepository.findOneWithEagerRelationships(id);
