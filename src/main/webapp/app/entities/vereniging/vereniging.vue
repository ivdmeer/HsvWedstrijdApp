      ...
      <table class="table table-striped" aria-describedby="verenigings">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('hsvWedstrijdApp.vereniging.persoon')">Persoon</span></th>
            <th scope="row"><span v-text="$t('hsvWedstrijdApp.vereniging.team')">Team</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="vereniging in verenigings" :key="vereniging.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="vereniging.persoon">
                <router-link :to="{ name: 'PersoonView', params: { persoonId: vereniging.persoon.id } }">{{
                  vereniging.persoon.id
                }}</router-link>
              </div>
            </td>
            <td>
              <div v-if="vereniging.team">
                <router-link :to="{ name: 'TeamView', params: { teamId: vereniging.team.id } }">{{ vereniging.team.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
