<template>
  <div>
    <h2 id="page-heading" data-cy="TeamHeading">
      <span v-text="$t('hsvWedstrijdApp.team.home.title')" id="team-heading">Teams</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hsvWedstrijdApp.team.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TeamCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-team">
            ...
            <span v-text="$t('hsvWedstrijdApp.team.home.createLabel')"> Create a new Team </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && teams && teams.length === 0">
      <span v-text="$t('hsvWedstrijdApp.team.home.notFound')">No teams found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="teams">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('hsvWedstrijdApp.team.naam')">Naam</span></th>
            <th scope="row"><span v-text="$t('hsvWedstrijdApp.team.vereniging')">Vereniging</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="team in teams" :key="team.id" data-cy="entityTable">
            ...
            <td>{{ team.naam }}</td>
            <td>
              <div v-if="team.vereniging">
                <router-link :to="{ name: 'VerenigingView', params: { verenigingId: team.vereniging.id } }">{{
                  team.vereniging.naam
                }}</router-link>
              </div>
            </td>
            <td class="text-right">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="hsvWedstrijdApp.team.delete.question" data-cy="teamDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-team-heading" v-text="$t('hsvWedstrijdApp.team.delete.question', { id: removeId })">
