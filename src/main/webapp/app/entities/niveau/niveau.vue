      ...
      <table class="table table-striped" aria-describedby="niveaus">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('hsvWedstrijdApp.niveau.score')">Score</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="niveau in niveaus" :key="niveau.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="niveau.score">
                <router-link :to="{ name: 'ScoreView', params: { scoreId: niveau.score.id } }">{{ niveau.score.id }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
