...
import nl.imvdm.hsv.contestapp.repository.ScoreRepository;
...
class ScoreResourceIT {
    ...
    private static final Double DEFAULT_PUNTEN = 1D;
    private static final Double UPDATED_PUNTEN = 2D;
    ...
    public static Score createEntity(EntityManager em) {
        Score score = new Score().punten(DEFAULT_PUNTEN);
        return score;
    }
    ...
    public static Score createUpdatedEntity(EntityManager em) {
        Score score = new Score().punten(UPDATED_PUNTEN);
        return score;
    }
    ...
    void createScore() throws Exception {
        ...
        assertThat(testScore.getPunten()).isEqualTo(DEFAULT_PUNTEN);
    }
    ...
    @Test
    @Transactional
    void checkPuntenIsRequired() throws Exception {
        int databaseSizeBeforeTest = scoreRepository.findAll().size();
        // set the field null
        score.setPunten(null);
    ...
    void getAllScores() throws Exception {
        ...
        restScoreMockMvc
            ...
            .andExpect(jsonPath("$.[*].punten").value(hasItem(DEFAULT_PUNTEN.doubleValue())));
    }

    @SuppressWarnings({ "unchecked" })
    void getAllScoresWithEagerRelationshipsIsEnabled() throws Exception {
        when(scoreRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restScoreMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(scoreRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getScore() throws Exception {
        ...
        restScoreMockMvc
            ...
            .andExpect(jsonPath("$.punten").value(DEFAULT_PUNTEN.doubleValue()));
    }
    ...
    void putNewScore() throws Exception {
        ...
        updatedScore.punten(UPDATED_PUNTEN);
        ...
        Score testScore = scoreList.get(scoreList.size() - 1);
        assertThat(testScore.getPunten()).isEqualTo(UPDATED_PUNTEN);
    }
    ...
    void fullUpdateScoreWithPatch() throws Exception {
        ...
        partialUpdatedScore.punten(UPDATED_PUNTEN);
