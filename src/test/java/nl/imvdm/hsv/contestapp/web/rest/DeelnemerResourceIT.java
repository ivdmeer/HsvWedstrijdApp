...
class DeelnemerResourceIT {
    ...
    @Mock
    private DeelnemerRepository deelnemerRepositoryMock;
    ...
    void getAllDeelnemersWithEagerRelationshipsIsEnabled() throws Exception {
        when(deelnemerRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDeelnemerMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(deelnemerRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllDeelnemersWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(deelnemerRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
